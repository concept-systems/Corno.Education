...
function LoadCalendarScript(callback) {
    function LoadFullCalendarScript() {
        if (!$.fn.fullCalendar) {
            $.getScript("/Content/plugins/fullcalendar/fullcalendar.js", callback);
        } else {
    ...
    if (!$.fn.moment) {
        $.getScript("/Content/plugins/moment/moment.min.js", LoadFullCalendarScript);
    } else {
...
function LoadBootstrapValidatorScript(callback) {
    if (!$.fn.bootstrapValidator) {
        $.getScript("/Content/plugins/bootstrapvalidator/bootstrapValidator.js", callback);
    } else {
        if (callback && typeof (callback) === "function") {
            callback();
        }
...
function LoadDataTablesScripts(callback) {
    ...
    function LoadDatatables() {
        $.getScript("/Content/plugins/datatables/jquery.dataTables.js",
            function() {
                $.getScript("/Content/plugins/datatables/ZeroClipboard.js",
                    function() {
                        $.getScript("/Content/plugins/datatables/TableTools.js",
                            function() {
                                $.getScript("/Content/plugins/datatables/dataTables.bootstrap.js", callback);
                            });
                    });
            });
    }
...
function LoadFineUploader(callback) {
    if (!$.fn.fineuploader) {
        $.getScript("/Content/plugins/fineuploader/jquery.fineuploader-4.3.1.min.js", callback);
    } else {
        if (callback && typeof (callback) === "function") {
            callback();
        }
...
function LoadXChartScript(callback) {
    function LoadXChart() {
        $.getScript("/Content/plugins/xcharts/xcharts.min.js", callback);
    }

    function LoadD3Script() {
        if (!$.fn.d3) {
            $.getScript("/Content/plugins/d3/d3.v3.min.js", LoadXChart);
        } else {
            LoadXChart();
        }
    }
...
function LoadFlotScripts(callback) {
    function LoadFlotScript() {
        $.getScript("/Content/plugins/flot/jquery.flot.js", LoadFlotResizeScript);
    }

    function LoadFlotResizeScript() {
        $.getScript("/Content/plugins/flot/jquery.flot.resize.js", LoadFlotTimeScript);
    }

    function LoadFlotTimeScript() {
        $.getScript("/Content/plugins/flot/jquery.flot.time.js", callback);
    }
...
function LoadMorrisScripts(callback) {
    function LoadMorrisScript() {
        if (!$.fn.Morris) {
            $.getScript("/Content/plugins/morris/morris.min.js", callback);
        } else {
            if (callback && typeof (callback) === "function") {
                callback();
            }
    ...
    if (!$.fn.raphael) {
        $.getScript("/Content/plugins/raphael/raphael-min.js", LoadMorrisScript);
    } else {
        LoadMorrisScript();
    }
}
...
function LoadFancyboxScript(callback) {
    if (!$.fn.fancybox) {
        $.getScript("/Content/plugins/fancybox/jquery.fancybox.js", callback);
    } else {
        if (callback && typeof (callback) === "function") {
            callback();
        }
...
function LoadKnobScripts(callback) {
    if (!$.fn.knob) {
        $.getScript("/Content/plugins/jQuery-Knob/jquery.knob.js", callback);
    } else {
        if (callback && typeof (callback) === "function") {
            callback();
        }
...
function LoadSparkLineScript(callback) {
    if (!$.fn.sparkline) {
        $.getScript("/Content/plugins/sparkline/jquery.sparkline.min.js", callback);
    } else {
        if (callback && typeof (callback) === "function") {
            callback();
        }
...
function SparkLineDrawBarGraph(elem, arr, color) {
    ...
    elem.sparkline(arr,
    { type: "bar", barWidth: 7, highlightColor: "#000", barSpacing: 2, height: 40, stackedBarColor: stacked_color });
}
...
function MorrisChart3() {
    ...
    for (var x = 0; x <= 360; x += 10) {
        decimal_data
            .push({ x: x, y: Math.sin(Math.PI * x / 180).toFixed(4), z: Math.cos(Math.PI * x / 180).toFixed(4) });
    }
    Morris.Line({
...
function TestTable3() {
    $("#datatable-3")
        .dataTable({
            "aaSorting": [[0, "desc"]],
            "sDom":
                "T<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
            "sPaginationType": "bootstrap",
            "oLanguage": {
                "sSearch": "",
                "sLengthMenu": "_MENU_"
            ...
            "oTableTools": {
                "sSwfPath": "/Content/plugins/datatables/copy_csv_xls_pdf.swf",
                "aButtons": [
                    "copy",
                    "print",
                    {
...
function LoadTestMap() {
    $.getJSON("http://www.telize.com/geoip?callback=?",
        function(json) {
            ...
            var googlesattelite = new OpenLayers.Layer
                .Google("Google Sattelite", { type: google.maps.MapTypeId.SATELLITE, numZoomLevels: 22 });
...
function CreateAllSliders() {
    ...
    slider_range_amount.val("$" +
        slider_range.slider("values", 0) +
        " - $" +
        slider_range.slider("values", 1));
    $("#equalizer > div.progress > div")
        .each(function() {
...
$(document)
    .ready(function() {
        ...
        if (ajax_url.length < 1) {
            ...
            $('a[href="' + this.location.pathname + '"]').parent().parent().addClass("active-parent");
            var parent_to_parent = $('a[href="' + this.location.pathname + '"]').parent().parent();
            var parents = $('a[href="' + this.location.pathname + '"]').parents("li");
            var li = $('a[href="' + this.location.pathname + '"]').closest("li.dropdown");
            var another_items = $(".main-menu li").not(parents);
            another_items.find("a").removeClass("active");
            another_items.find("a").removeClass("active-parent");
            if (parent_to_parent.is(":visible")) {
                li.find("ul.dropdown-menu").slideUp("fast");
                li.find("ul.dropdown-menu a").removeClass("active");
            } else {
                another_items.find("ul.dropdown-menu").slideUp("fast");
                parent_to_parent.slideDown("fast");
            }
        } else
