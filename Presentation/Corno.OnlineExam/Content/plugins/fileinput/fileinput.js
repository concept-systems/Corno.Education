...
(function($) {
    ...
    var isIE = function(ver) {
        ...
        defaultFileTypeSettings = {
            ...
            video: function(vType, vName) {
                return (vType !== undefined && vType.match(/\.video\/(ogg|mp4|webm)$/i)) ||
                    vName.match(/\.(og?|mp4|webm)$/i);
            },
            audio: function(vType, vName) {
                return (vType !== undefined && vType.match(/\.audio\/(ogg|mp3|wav)$/i)) ||
    ...
    FileInput.prototype = {
        ...
        listen: function() {
            ...
            self.$container.find(".kv-fileinput-upload")
                .off("click")
                .on("click",
                    function(e) {
                        var $btn = $(this),
        ...
        initFileActions: function() {
            var self = this;
            self.$preview.find(".kv-file-remove")
                .each(function() {
                    var $el = $(this),
                                ...
                                $frame.fadeOut("slow",
                                    function() {
                                        ...
                                        if (len === 0 && chk === 0) {
                                            ...
                                            n = chk + len;
                                            cap = n > 1
                                                ? self.getMsgSelected(n)
                                                : (filestack[0] ? filestack[0].name : "");
                                            self.setCaption(cap);
                                        }
                                    });
                            });
                });
            self.$preview.find(".kv-file-upload")
                .each(function() {
                    var $el = $(this);
                    $el.off("click")
                        .on("click",
        ...
        initSuccessThumbs: function() {
            var self = this;
            self.getThumbs(".file-preview-success")
                .each(function() {
                    ...
                    $remove.removeAttr("disabled")
                        ...
                        .on("click",
                            function() {
                                var out = self
                                    .raise("filesuccessremove", [$thumb.attr("id"), $thumb.data("fileindex")]);
        ...
        previewDefault: function(file, previewId, isDisabled) {
            ...
            if (isDisabled === true) {
                ...
                footer +=
                    '<div class="file-other-error text-danger"><i class="glyphicon glyphicon-exclamation-sign"></i></div>';
            }
            self.$preview.append("\n" +
        ...
        previewFile: function(i, file, theFile, previewId, data) {
            ...
            if (chkTypes && chkMimes) {
                if (cat === "text") {
                    ...
                    content = tmplt.repl("{previewId}", previewId)
                        .repl("{caption}", caption)
                        .repl("{frameClass}", "")
                        .repl("{type}", file.type)
                        .repl("{width}", config.width)
        ...
        readFiles: function(files) {
            ...
            function readFile(i) {
                ...
                if (fileCount === 0 && !isEmpty(fileExt) && isArray(fileExt) && !isEmpty(fileExtExpr)) {
                    ...
                    fileCount += isEmpty(chk) ? 0 : chk.length;
                    if (fileCount === 0) {
                        msg = self.msgInvalidFileExtension.replace("{name}", caption)
                            .replace("{extensions}",
                                strExt);
                        self.isError = throwError(msg, file, previewId, i);
                        return;
        ...
        renderUpload: function() {
            ...
            if (!self.isUploadable || self.isDisabled) {
                content = '<button type="submit" title="' +
                    self.uploadTitle +
                    '"class="' +
                    css +
                    '"' +
