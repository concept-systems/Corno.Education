...
public partial class StudentResultRpt : BaseReport
{
    ...
    private static string GetResult(Tbl_STUDENT_COURSE studentCourse, TBl_STUDENT_ENV_STUDIES envStudies)
    {
        if (studentCourse.Chr_EnviStudies_Exempted == "Y")
            return "Exempted";
        if (null == envStudies)
            return "Not Attempted";
        if (envStudies.Chr_ST_SUB_RES == "P")
            return "Pass";
        if (envStudies.Chr_ST_SUB_RES != "P")
            return "Not Cleared";
        return "NA";
    ...
    private void HandleReport(object sender)
    {
        ...
        var dataSource = (from studentCourse in studentCourses
                          ...
                          join envStudies in examService.TBl_STUDENT_ENV_STUDIES_Repository.GetAll()
                              on studentCourse.Chr_FK_PRN_NO equals envStudies.Chr_FK_PRN_NO into defaultEnv
                          from envStudies in defaultEnv.DefaultIfEmpty()
                          select new
                          {
                              CollegeId = collegeId,
                              ...
                              Result = GetResult(studentCourse, envStudies),
                              PassMonth = envStudies?.Num_MONTH_NO,
                              PassYear = envStudies?.Num_YEAR
                          }).ToList();
