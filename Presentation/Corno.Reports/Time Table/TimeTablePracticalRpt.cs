...
public partial class TimeTablePracticalRpt : Report
{
    #region -- Connstructors --
    public TimeTablePracticalRpt(ICornoService transactionService)
    {
    ...
    #region -- Events --
    private void TimeTablePracticalRpt_NeedDataSource(object sender, EventArgs e)
    {
        ...
        var timeTables = _transactionService.TimeTableRepository.Get(t => t.InstanceId == _instanceId &&
                ...
                null, "TimeTablePracticalDetails")
            .AsQueryable()
            .AsNoTracking()
            .ToList();
        ...
        {
            dataSource.AddRange(timeTable.TimeTablePracticalDetails
                .Where(t => practicalCategories.Contains(t.CategoryCode ?? 0) && t.StartDate != null).Select(t =>
                    new
                    {
