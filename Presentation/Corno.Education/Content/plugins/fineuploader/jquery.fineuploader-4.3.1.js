...
//NOTE: will also return true for touch-only devices.  It's not currently possible to accurately test for touch-only devices
...
(function() {
    ...
    qq.basePublicApi = {
                ...
                qq.each(blobDataArray,
                    function(idx, blobData) {
                        ...
                        if (qq.isBlob(blobData) && !qq.isFileOrInput(blobData)) {
                            ...
                            self
                                .log("addBlobs: entry at index " + idx + " is not a Blob or a BlobData object",
                                    "error");
                        }
        ...
        pauseUpload: function(id) {
            ...
            if (qq.indexOf([qq.status.UPLOADING, qq.status.UPLOAD_RETRYING], uploadData.status) >= 0) {
                ...
                qq.log(qq.format("Ignoring pause for file ID {} ({}).  Not in progress.", id, this.getName(id)),
                    "error");
            }
    ...
    qq.basePrivateApi = {
            ...
            if (qq.Session && this._options.session.endpoint != null) {
                ...
                setTimeout(function() {
                        self._session.refresh()
                            .then(function(response, xhrOrXdr) {
        ...
        _addCannedFile: function(sessionData) {
            var id = this._uploadData.addFile(sessionData.uuid,
                sessionData.name,
                sessionData.size,
        ...
        _onSubmitDelete: function(id, onSuccessCallback, additionalMandatedParams) {
            ...
            if (this._isDeletePossible()) {
                ...
                this.log("Delete request ignored for ID " +
                    id +
        ...
        _onValidateBatchCallbackFailure: function(fileWrappers) {
            ...
            qq.each(fileWrappers,
                function(idx, fileWrapper) {
                ...
                if (validItem || !this._options.validation.stopOnFirstInvalidFile) {
                    ...
                    setTimeout(function() {
                            ...
                            self._handleCheckedCallback({
                                ...
                                onSuccess: qq.bind(self._onValidateCallbackSuccess,
                                    self,
                                    items,
                                    index,
                                    params,
                                    endpoint),
                                onFailure: qq.bind(self._onValidateCallbackFailure,
                                    self,
                                    items,
                                    index,
                                    params,
                                    endpoint),
                                identifier: "Item '" +
                                    validationDescriptor.name +
                                    "', size: " +
                                    validationDescriptor.size
                            });
                        },
                        0);
                } else if (!validItem) {
        ...
        _validateFileOrBlobData: function(fileWrapper, validationDescriptor) {
            ...
            if (qq.ImageValidation && qq.supportedFeatures.imagePreviews && qq.isFile(file)) {
                new qq.ImageValidation(file, qq.bind(self.log, self)).validate(validationBase.image)
                    .then(
        ...
        _parseFileOrBlobDataName: function(fileOrBlobData) {
            ...
            if (qq.isFileOrInput(fileOrBlobData)) {
                if (fileOrBlobData.value) {
                    ...
                    name = (fileOrBlobData.fileName !== null && fileOrBlobData.fileName !== undefined)
                        ? fileOrBlobData.fileName
                        : fileOrBlobData.name;
                }
            } else {
        ...
        _parseFileOrBlobDataSize: function(fileOrBlobData) {
            ...
            if (qq.isFileOrInput(fileOrBlobData)) {
                if (fileOrBlobData.value === undefined) {
                    // fix missing properties in Safari 4 and firefox 11.0a2
                    size = (fileOrBlobData.fileSize !== null && fileOrBlobData.fileSize !== undefined)
                        ? fileOrBlobData.fileSize
                        : fileOrBlobData.size;
                }
            } else {
...
qq.AbstractUploadHandlerForm = function(spec) {
    ...
    function expungeFile(id) {
        ...
        if (iframe) {
            ...
            iframe
                .setAttribute("src", "java" + String.fromCharCode(115) + "cript:false;");
//deal with "JSLint: javascript URL" warning, which apparently cannot be turned off
    ...
    function registerPostMessageCallback(iframe, callback) {
        ...
        detachLoadEvents[fileId] = qq(iframe)
            .attach("load",
                function() {
                    if (fileState[fileId].input) {
                        ...
                        postMessageCallbackTimers[iframeName] = setTimeout(function() {
                                var errorMessage = "No valid message received from loaded iframe for iframe name " +
                                    iframeName;
    ...
    qq.extend(this,
    {
        ...
        _attachLoadEvent: function(iframe, callback) {
            ...
            if (isCors) {
                ...
                detachLoadEvents[iframe.id] = qq(iframe)
                    .attach("load",
                        function() {
                            ...
                            try {
                                ...
                                log("Error when attempting to access iframe during handling of upload response (" +
                                    error.message +
                                    ")",
                                    "error");
    ...
    qq.uiPublicApi = {
        ...
        reset: function() {
            ...
            if (!this._options.button && this._templating.getButton()) {
                this._defaultButtonId = this._createUploadButton({ element: this._templating.getButton() })
                    .getButtonId();
            }
...
qq.Templating = function(spec) {
    ...
    function cacheThumbnailPlaceholders() {
        ...
        if (showThumbnails) {
            if (notAvailableUrl) {
                options.imageGenerator.generate(notAvailableUrl, new Image(), spec)
                    .then(
                        function(updatedImg) {
            ...
            if (waitingUrl) {
                options.imageGenerator.generate(waitingUrl, new Image(), spec)
                    .then(
                        function(updatedImg) {
        ...
        generatePreview: function(id, fileOrBlob) {
            ...
            if (qq.supportedFeatures.imagePreviews) {
                ...
                if (thumbnail) {
                    ...
                    return options.imageGenerator.generate(fileOrBlob, thumbnail, spec)
                        .then(
                            function() {
                                show(thumbnail);
        ...
        updateThumbnail: function(id, thumbnailUrl, showWaitingImg) {
            ...
            if (thumbnail) {
                if (thumbnailUrl) {
                    ...
                    return options.imageGenerator.generate(thumbnailUrl, thumbnail, spec)
                        .then(
                            function() {
                                show(thumbnail);
                            },
...
qq.UploadHandlerForm = function(options, proxy) {
    ...
    qq.extend(this,
        new qq.AbstractUploadHandlerForm({
                options: {
    ...
    qq.extend(this,
    {
        upload: function(id) {
            ...
            handler._attachLoadEvent(iframe,
...
qq.UploadHandlerXhr = function(spec, proxy) {
    ...
    function uploadNextChunk(id) {
        ...
        log("Sending chunked upload request for item " +
            id +
            ": bytes " +
            (chunkData.start + 1) +
            "-" +
    ...
    function handleResetResponseOnResumeAttempt(id) {
        handler._getFileState(id).attemptingResume = false;
        log("Server has declared that it cannot handle resume for item ID " + id + " - starting from the first chunk",
            "error");
    ...
    function calculateRemainingChunkIdxsAndUpload(id, firstChunkIndex) {
        ...
        for (currentChunkIndex = handler._getTotalChunks(id) - 1;
            currentChunkIndex >= firstChunkIndex;
            currentChunkIndex -= 1) {
    ...
    qq.extend(this,
        new qq.AbstractUploadHandlerXhr({
                options: {
            ...
            return {
                ...
                getResumableFilesData: function() {
                    ...
                    if (chunkFiles && resumeEnabled) {
                        if (resumeId === undefined) {
                            matchingCookieNames = qq.getCookieNames(new RegExp("^qqfilechunk\\" +
                                cookieItemDelimiter +
                                ".+\\" +
                                cookieItemDelimiter +
                                "\\d+\\" +
                                cookieItemDelimiter +
                                spec.chunking.partSize +
                                "="));
                        } else {
                            matchingCookieNames = qq.getCookieNames(new RegExp("^qqfilechunk\\" +
                                cookieItemDelimiter +
...
qq.DragAndDrop = function(o) {
    ...
    function traverseFileTree(entry) {
        ...
        if (entry.isFile) {
            entry.file(function(file) {
                ...
                function(fileError) {
                    options.callbacks.dropLog("Problem parsing '" +
                        entry.fullPath +
                        "'.  FileError code " +
                        fileError.code +
                        ".",
                    ...
                    function(idx, item) {
                        ...
                        if (entry) {
                            ...
                            if (entry.isFile) {
                                ...
                                pendingFolderPromises.push(traverseFileTree(entry)
                                    .done(function() {
    ...
    function setupDropzone(dropArea) {
        var dropZone = new qq.UploadDropZone({
            ...
            onDrop: function(e) {
                handleDataTransfer(e.dataTransfer, dropZone)
                    .then(
                        function() {
                        ...
                        function() {
                            options.callbacks
                                .dropLog("Drop event DataTransfer parsing failed.  No files will be uploaded.",
                                    "error");
                        }
                    );
            }
    ...
    function setupDragDrop() {
        ...
        qq.each(dropZones,
            function(idx, dropZone) {
                ...
                if (dropZones.length && (!qq.ie() || qq.ie10())) {
                    disposeSupport.attach(document,
                        "dragenter",
                        function(e) {
        ...
        disposeSupport.attach(document,
            ...
            function(e) {
                qq.each(options.dropZoneElements,
                    function(idx, zone) {
                    ...
                    if (registerThumbnailRenderedListener(container, drawPreview)) {
                        exif.parse()
                            .then(
                                ...
                                function(failureMsg) {
                                    log(qq
                                        .format("EXIF data could not be parsed ({}).  Assuming orientation = 1.",
                                            failureMsg));
...
qq.Exif = function(fileOrBlob, log) {
    ...
    function seekToApp1(offset, promise) {
        ...
        qq.readBlobToHex(fileOrBlob, theOffset, 4)
            .then(function(hex) {
    ...
    function getApp1Offset() {
        ...
        qq.readBlobToHex(fileOrBlob, 0, 6)
            .then(function(hex) {
    ...
    function isLittleEndian(app1Start) {
        ...
        qq.readBlobToHex(fileOrBlob, app1Start + 10, 2)
            .then(function(hex) {
    ...
    function getDirEntryCount(app1Start, littleEndian) {
        ...
        qq.readBlobToHex(fileOrBlob, app1Start + 18, 2)
            .then(function(hex) {
                if (littleEndian) {
...
qq.Identify = function(fileOrBlob, log) {
    ...
    qq.extend(this,
    {
        isPreviewable: function() {
            ...
            qq.readBlobToHex(fileOrBlob, 0, 4)
                .then(function(hex) {
...
qq.ImageValidation = function(blob, log) {
    ...
    function getWidthHeight() {
        ...
        new qq.Identify(blob, log).isPreviewable()
            .then(function() {
                    var image = new Image(),
...
qq.Session = function(spec) {
    ...
    function handleFileItems(fileItems, success, xhrOrXdr, promise) {
        ...
        if (success) {
            qq.each(fileItems,
                function(idx, fileItem) {
                    /* jshint eqnull:true */
                    if (fileItem.uuid == null) {
                        someItemsIgnored = true;
                        options.log(qq.format("Session response item {} did not include a valid UUID - ignoring.", idx),
                            "error");
                    } else if (fileItem.name == null) {
                        someItemsIgnored = true;
                        options.log(qq.format("Session response item {} did not include a valid name - ignoring.", idx),
                            "error");
                    } else {
                        try {
        ...
        qq.each(buttonHandlers,
            function(buttonType, handler) {
                ...
                if (spec.templating["is" + firstLetterCapButtonType](target)) {
                    ...
                    spec.log(qq.format("Detected valid file button click event on file '{}', ID: {}.",
    ...
    function examineEvent(target, event) {
        if (spec.templating.isFileName(target) || spec.templating.isEditIcon(target)) {
            ...
            if (status === qq.status.SUBMITTED) {
                spec.log(qq.format("Detected valid filename click event on file '{}', ID: {}.",
                    spec.onGetName(fileId),
                    fileId));
                qq.preventDefault(event);
...
qq.FilenameInputFocusInHandler = function(s, inheritedInternalApi) {
    ...
    function handleInputFocus(target, event) {
        if (spec.templating.isEditInput(target)) {
            ...
            if (status === qq.status.SUBMITTED) {
                spec.log(qq.format("Detected valid filename input focus event on file '{}', ID: {}.",
                    spec.onGetName(fileId),
                    fileId));
...
qq.FilenameEditHandler = function(s, inheritedInternalApi) {
    ...
    function registerInputBlurHandler(inputEl, fileId) {
        inheritedInternalApi.getDisposeSupport()
            .attach(inputEl,
                "blur",
                function() {
