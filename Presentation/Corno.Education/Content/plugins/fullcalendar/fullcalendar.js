...
FCMoment.prototype.time = function(time) {
    if (time == null) { // getter
        ...
        return this.hours(time
                .hours() +
                time.days() * 24) // day value will cause overflow (so 24 hours becomes 00:00:00 of next day)
                            ...
                            if (!cell.row) { // on full-days
                                ...
                                if (isStart) {
                                    if (allDay) {
                                        ...
                                        setOuterHeight(eventElement,
                                            calendar
                                            .defaultTimedEventDuration /
                                            slotDuration *
                                            slotHeight); // the default height
            ...
            stop: function(ev, ui) {
                ...
                if (revert) { // hasn't moved or is out of bounds (draggable has already reverted)
                    ...
                    var eventStart = event.start
                        .clone()
                        .add("days", dayDelta); // already assumed to have a stripped time
                    var snapTime;
                    var snapIndex;
                    if (!allDay) {
                        snapIndex = Math.round((eventElement.offset().top - getSlotContainer().offset().top) /
                            snapHeight); // why not use ui.offset.top?
...
function DayEventRenderer() {
    ...
    function resizableDayEvent(event, element, segment) {
        ...
        var handle = element
            .find(".ui-resizable-" + direction); // TODO: stop using this class because we aren't using jqui for this
