...
public static class GridHelper
{
    ...
    ) where T : class
    {
        ...
        return grid
            .HtmlAttributes(new { style = $"width: auto;{alignmentStyle}" })
            .ToolBar(toolbar =>
            {
            ...
            .Excel(excel => excel.AllPages(true).FileName("Export.xlsx"))
            .Pdf(pdf => pdf.AllPages().FileName("Export.pdf").AvoidLinks(true).PaperSize("A4").RepeatHeaders(true)
                .Landscape())
    ...
    {
        columns.Bound(ModelConstants.Code);
        columns.Bound(ModelConstants.Name);
        columns.Bound(ModelConstants.Description);
    }
...
namespace Corno.Concept.Portal.Areas.Helpers;
