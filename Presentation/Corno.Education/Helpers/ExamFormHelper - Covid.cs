...
namespace Corno.Education.Helpers
{
    public class ExamFormHelper
    {
        public static IEnumerable<ExamSubjectViewModel> FreshSubjects(int coursePartId, int branchId)
        {
            ...
            var syllabuses =
                examService.Tbl_COPART_SYLLABUS_Repository.Get(
                    s => s.Num_FK_COPRT_NO == coursePartId && s.Num_FK_BR_CD == branchId && s.Chr_DELETE_FLG != "Y");
            if (null == syllabuses)
                return null;
        ...
        public static IEnumerable<ExamSubjectViewModel> FailSubjects(int instanceNo, int coursePartId, string prnNo)
        {
            ...
            var coursePart =
                examService.Tbl_COURSE_PART_MSTR_Repository.Get(c => c.Num_PK_COPRT_NO == coursePartId)
                    .FirstOrDefault();
            if (coursePart != null && coursePart.Chr_FAIL_ALL_CATEGORIES == "F")
            {
                var allSubjects =
            ...
            if (coursePart.Num_COPRT_MAX_ATMPTS_TO_CMPLT > 0 &&
                previousAttemtsCount >= coursePart.Num_COPRT_MAX_ATMPTS_TO_CMPLT)
            ...
            var passSubjects = examService.Tbl_STUDENT_SUBJECT_Repository.Get(
                    s =>
        ...
        private static void GetFeeStructure(ExamViewModel viewModel, int instanceId)
        {
            var examService = (ICoreService)Bootstrapper.GetService(typeof(CoreService));

            if (viewModel.CourseId == null) return;
            if (viewModel.CoursePartId == null) return;
            if (viewModel.BranchId == null) return;

            var feeStructure = ExamServerHelper.GetFeeStructureForExam(instanceId, (int)viewModel.CourseId,
                (int)viewModel.CoursePartId, true, examService);
            ...
            if (null != yrChange)
            {
                // TODO: (Remove it) Temporary arrangement for COVID 
                //var coursePartFail = examService.Tbl_COURSE_PART_MSTR_Repository.Get(c => c.Num_PK_COPRT_NO ==
                //                                                                          viewModel.CoursePartId)
                //    .FirstOrDefault();
                //var nextCoursePart = ExamServerHelper.GetNextCoursePart(viewModel.CoursePartId ?? 0, examService);
                if ((yrChange.Chr_ST_RESULT == "F" || yrChange.Chr_ST_RESULT == "O" ||
                    yrChange.Chr_ST_RESULT == "T") && 
                    viewModel.CoursePartId == yrChange.Num_FK_COPRT_NO)
                {
                ...
                var subjects =
                    FailSubjects(yrChange.Num_FK_INST_NO, yrChange.Num_FK_COPRT_NO, viewModel.PrnNo).ToList();
                ...
                {
                    var fee = ExamServerHelper.GetFeeStructureForExam(instanceId, (int)viewModel.CourseId,
                        line.CoursePartID ?? 0, false, examService);
                    ...
                    if (line.Count <= 3)
                    {
                        ...
                        if (null != coursePart && coursePart.Num_COPRT_TOT_SUBJECT <= 3 &&
            ...
            if (viewModel.CoursePartId != null && viewModel.BranchId != null)
            {
                viewModel.ExamSubjectViewModels =
                    GetExamSubjects(viewModel.PrnNo, (int)viewModel.CoursePartId,
                        (int)viewModel.BranchId).ToList();

                var coursePartCount = viewModel.ExamSubjectViewModels.Where(e => e.CoursePartId != 1247)
            ...
            // Calculate total.
            viewModel.Total = feeStructure.GetTotal();
            var amountInWordService = (IAmountInWordsService)Bootstrapper.GetService(typeof(AmountInWordsService));
            viewModel.FeeInWord = amountInWordService.GetAmountInWords(viewModel.Total.ToString());

            //string amt = feeStructure.Total.ToString();
            //var feeInWord = ExamServerHelper.GetAmountInWords(amt);
        }
        ...
        public static void GetPrnInfo(ExamViewModel viewModel)
        {
            ...
            if (null == yearChangeRecord)
            {
                ...
                if (null != coursePartFirst && coursePartFirst.Chr_COPRT_BRANCH_APP_FLG == "Y")
                {
            ...
            else
            {
                ...
                if (null != viewModel.CoursePartId && viewModel.CoursePartId > 0)
                {
                    nextCoursePart =
                ...
                if (null != nextCoursePart && nextCoursePart.Chr_COPRT_BRANCH_APP_FLG == "Y")
                {
            ...
            // Get Cousre Part from Tbl_COURSE_PART_MSTR
            ...
            var coursePart =
                examService.Tbl_COURSE_PART_MSTR_Repository.Get(c => c.Num_PK_COPRT_NO == viewModel.CoursePartId)
                    .FirstOrDefault();
            if (coursePart != null && coursePart.Chr_DEG_APL_FLG == "Y")
