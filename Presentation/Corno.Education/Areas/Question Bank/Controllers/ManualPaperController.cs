...
using Corno.Logger;
using Corno.Education.Areas.Question_Bank.Dtos;
...
namespace Corno.Education.Areas.Question_Bank.Controllers;

[Authorize]
public class ManualPaperController : CornoController
{
    #region -- Constructors --
    public ManualPaperController(IRichEditDocumentService richEditDocumentService,
        ISubjectService subjectService, IMiscMasterService miscMasterService)
    {
        ...
        const string viewPath = "~/Areas/question bank/views/ManualPaper/";
    ...
    public ActionResult Preview(Paper paper)
    {
        ...
        try
        {
            ...
            /*// Download as pdf
            var bytes = ConvertToPdf(document);
            //Send the File to Download.
            //return File(bytes, "application/octet-stream", "Paper.pdf");
            ViewBag.PdfBytes = bytes;
            return View(_previewPath);*/

            /*// Show in HTML
            var htmlContent = ConvertToHtml(document);
