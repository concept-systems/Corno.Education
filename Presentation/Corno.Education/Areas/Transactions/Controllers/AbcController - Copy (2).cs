        ...
        public ActionResult Create(UniversityBaseModel viewModel)
        {
            try
            {
                ...
                join subject in _coreService.Tbl_STUDENT_SUBJECT_Repository.Get(s => s.Num_FK_INST_NO == instanceId &&
                        s.Num_FK_COPRT_NO == viewModel.CoursePartId) on exam.Chr_FK_PRN_NO equals subject.Chr_FK_PRN_NO
                ...
                from grade in defaultGrade.DefaultIfEmpty()
                             select new
                    {
                        exam.Num_FK_INST_NO, exam.Chr_FK_PRN_NO, exam.Num_ST_SEAT_NO, exam.Num_FK_CO_CD, exam.Num_FK_COPRT_NO, 
                        exam.Num_FK_BR_CD, exam.Num_FK_CLASS_CD, exam.Num_COPRT_SEMI_NO,
                        subject.Num_FK_SUB_CD, grade?.Num_PK_GRD_CD, GradeName = grade?.Var_GRD_NM, GradePoints = grade?.Num_GRD_POINTS,
                    }).ToList();
                var courseIds = exams.Select(s => s.Num_FK_CO_CD).Distinct();
                var courses = _coreService.Tbl_COURSE_MSTR_Repository.Get(c =>
                    courseIds.Contains(c.Num_PK_CO_CD)).ToList();
                var coursePartIds = exams.Select(s => s.Num_FK_COPRT_NO).Distinct();
                var courseParts = _coreService.Tbl_COURSE_PART_MSTR_Repository.Get(c =>
                    coursePartIds.Contains(c.Num_PK_COPRT_NO)).ToList();
                var branchIds = exams.Select(s => s.Num_FK_BR_CD).Distinct();
                ...
                var dataSource = from exam in exams
                                 ...
                                 join subject in subjects on exam.Num_FK_SUB_CD equals subject.Num_PK_SUB_CD
                                 join studentInfo in studentInfos on exam.Chr_FK_PRN_NO equals studentInfo.Chr_PK_PRN_NO
                                 join studentInfoAdr in studentInfoAdrs on exam.Chr_FK_PRN_NO equals studentInfoAdr.Chr_FK_PRN_NO
                                 join schedule in _coreService.Tbl_EXAM_SCHEDULE_MSTR_Repository.Get()
                                 ...
                                 join branch in branches on exam.Num_FK_BR_CD  equals branch.Num_PK_BR_CD into defaultBranch
                                 ...
                                 select new AbcExportModel()
                                 {
                                     ...
                                     DateOfBirth = string.IsNullOrEmpty(studentInfo?.Dtm_ST_DOB_DT) ? $"{new DateTime(1900, 1,1):dd/MMM/yyyy}" 
                                     ...
                                     TotalGradePoints = exam.GradePoints,   // ?
                                     ...
                                     SubjectGrade = exam.GradeName,
                                     SubjectGradePoints = exam.GradePoints,

                                     SubjectCredit = subject.Num_SUBJECT_CREDIT,
                                     SubjectCreditPoints = exam.GradePoints * subject.Num_SUBJECT_CREDIT
                                 };
                // Make distinct
                dataSource = dataSource.DistinctBy(d => new {d.Prn, d.SubjectCode});
