...
public class RevaluationController : BaseController
{
    ...
    private void GetSeatNoInfo(RevalutionViewModel viewModel)
    {
        ...
        var revaluationDate = ExamServerHelper.GetRevalStartDate(_examService, instanceId, yearChangeRecord.Num_FK_COPRT_NO);
        if (null == revaluationDate)
            throw new Exception("Revaluation Start Date is not available for Instance (" + instanceId +
                                ") and Course Part (" + yearChangeRecord.Num_FK_COPRT_NO + ")");

        if (DateTime.Now.Date > revaluationDate.Value.Date)
            throw new Exception("Revaluation Start Date (" + revaluationDate.Value.ToString("dd/MM/yyyy") + ") is over for Instance (" + instanceId +
                                ") and Course Part (" + yearChangeRecord.Num_FK_COPRT_NO + ")");
