...
namespace Corno.Education.Areas.Transactions.Controllers
{
    [Authorize]
    public class MarksEntryController : UniversityController
    {
        #region -- Constructors --
        public MarksEntryController(ICornoService cornoService, ICoreService coreService,
            IMarksEntryService marksEntryService,
            IExcelFileService<MarksImportModel> excelFileService)
        : base(cornoService, coreService)
        {
            _coreService = coreService;
            _marksEntryService = marksEntryService;
            _excelFileService = excelFileService;

            _viewPath = "~/Areas/transactions/views/MarksEntry/Create.cshtml";
        }
        #endregion
            ...
            if (model.PaperId.ToInt() <= 0)
            {
                ...
                marksEntryDetails = (from exam in _coreService.Tbl_STUDENT_SUBJECT_Repository.Get(e =>
                        e.Num_FK_INST_NO == instanceId && e.Num_FK_COPRT_NO == (model.CoursePartId ?? 0) && e.Num_FK_SUB_CD == model.SubjectId &&
                                                  e.Chr_ST_SUB_STS == "A" && e.Chr_ST_SUB_CAN != "Y"/*&& (e.Num_FK_BR_CD ?? 0) == (model.BranchId ?? 0)#1#)
                                     ...
                                     select new MarksEntryDetail
                                     {
                                         ...
                                         MaxMarks = category?.Num_CAT_MAX_MRK ?? 0
                                     }).ToList();
            }
            else
            {
                ...
                marksEntryDetails = (from exam in _coreService.Tbl_STUDENT_SUBJECT_Repository.Get(e =>
                        ...
                        e.Chr_ST_SUB_STS == "A" && e.Chr_ST_SUB_CAN != "Y")
                                     ...
                                     select new MarksEntryDetail
                                     {
                                         Prn = exam.Chr_FK_PRN_NO,
                                         SeatNo = yrChange.Num_ST_SEAT_NO,
                                         MaxMarks = paper?.Num_PAP_MAX_MRK ?? 0
                                     }).ToList();
            }
        ...
        public ActionResult Create()
        {
            ...
            marksEntry.InstanceId = sessionData?.InstanceId;
            marksEntry.CollegeId = sessionData?.CollegeId ?? -1;

            return View(_viewPath, marksEntry);
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    var user = new AspNetUser
                        { Id = User.Identity.GetUserId(), UserName = User.Identity.GetUserName() };

                    _marksEntryService.Save(model, submitType, user);
        ...
        [Authorize]
        [ValidateAntiForgeryToken]
        [MultipleButton(Name = "action", Argument = "FindStudents")]
        public ActionResult FindStudents(MarksEntry marksEntry)
        {
            if (!ModelState.IsValid)
                return View(_viewPath, marksEntry);
            ...
            try
            {
                ValidateFields(marksEntry, ModelConstants.Search);
                // Get new subjects.
                marksEntry.MarksEntryDetails?.Clear();

                // Get Seat Nos.
                _marksEntryService.GetFormSeatNos(marksEntry);

                marksEntry.bEnable = false;
                //return View(model);
                return View(_viewPath, marksEntry);
            }
            catch (Exception exception)
        ...
        [Authorize]
        [ValidateAntiForgeryToken]
        [MultipleButton(Name = "action", Argument = "ImportFromEklavya")]
        public ActionResult ImportFromEklavya(MarksEntry marksEntry)
        {
            if (!ModelState.IsValid)
                return View(_viewPath, marksEntry);
            ...
            try
            {
                // Import from API from Eklavya
                _marksEntryService.ImportFromEklavya(marksEntry);
            }
            catch (Exception exception)
            {
                HandleControllerException(exception);
        ...
        [Authorize]
        [ValidateAntiForgeryToken]
        [MultipleButton(Name = "action", Argument = "SlipReport")]
        public ActionResult SlipReport(MarksEntry marksEntry)
        {
            if (!ModelState.IsValid)
                return View(_viewPath, marksEntry);
            ...
            try
            {
                ...
                // Get Seat Nos.
                _marksEntryService.GetReportSeatNos(marksEntry);

                if (marksEntry.MarksEntryDetails?.Count <= 0)
                    throw new Exception("No entries found for marks entry.");

                Session[ModelConstants.Report] = new MarksSlipRpt(marksEntry);
                return RedirectToAction("Details", "Report", new { area = "Reports", reportName = nameof(MarksSlipRpt), description = string.Empty });
            }
            catch (Exception exception)
            {
        ...
        [Authorize]
        [ValidateAntiForgeryToken]
        [MultipleButton(Name = "action", Argument = "SummaryReport")]
        public ActionResult SummaryReport(MarksEntry marksEntry)
        {
            if (!ModelState.IsValid)
                return View(_viewPath, marksEntry);
