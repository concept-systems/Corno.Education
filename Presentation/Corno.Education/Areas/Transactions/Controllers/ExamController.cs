...
public class ExamController : UniversityController
{
        ...
        FormType formType)
    {
        ...
        var callingMethodName = callingMethod != null ? $"{callingMethod.DeclaringType?.FullName}.{callingMethod.Name}" : "Unknown";
        LogHandler.LogInfo($"PaymentSuccess invoked by: {callingMethodName}");
        ...
        var existingFormInCore = _coreService.Tbl_APP_TEMP_Repository
            .Get(a => a.Num_FK_INST_NO == instanceId && a.Chr_APP_PRN_NO == prn && a.Num_Transaction_Id == transactionId)
            .FirstOrDefault();
        LogHandler.LogInfo($"existing in core : {existingFormInCore == null}, Payment Success : Instance : {instanceId}, PRN : {prn}, Transaction Id : {transactionId}");
        if (null == existingFormInCore)
        {
            LogHandler.LogInfo("Adding in App_Temp.");
