...
public class RichEditDocumentService : BaseService, IRichEditDocumentService
{
    ...
    private void CreateSections(Paper paper, Document document)
    {
        ...
        var sectionNos = structure.StructureDetails.Select(d => d.SectionNo ?? 0)
            .Distinct()
            .ToList();
        var miscMasterService = Bootstrapper.Bootstrapper.Get<IMiscMasterService>();
                    ...
                    case 1:
                        ...
                        switch ((QuestionType)(structureDetail.QuestionTypeId ?? 0))
                        {
                            ...
                            case QuestionType.ShortNotes:
                            {
                                ...
                                var firstOrDefault = paperDetails.FirstOrDefault();
                                coNo = GetCoNoString(firstOrDefault?.CoNo ?? 0, firstOrDefault?.Taxonomy ?? 0);// $"CO{firstOrDefault?.CoNo} BT{}";
                                AddQuestionRow(document, tableSections, structureDetail, attemptText, coNo,
                                    paperDetails.Count);
                            ...
                            case QuestionType.Mcq:
                                ...
                                foreach (var paperDetail in paperDetails)
                                {
                                    ...
                                    coNo = GetCoNoString(paperDetail?.CoNo ?? 0, paperDetail?.Taxonomy ?? 0); //$"CO{paperDetail.CoNo} BT{paperDetail.Taxonomy}";
                                    AddMcqRow(document, tableSections, paperDetail, htmlText, coNo);
                                }
                                break;
                            default:
                                for (var index = 0; index < paperDetails.Count; index++)
                                {
                                    ...
                                    var htmlText = ConvertByteArrayToHtml(paperDetail.DocumentContent);
                                    coNo = GetCoNoString(paperDetail?.CoNo ?? 0, paperDetail?.Taxonomy ?? 0); //$"CO{paperDetail?.CoNo} BT{paperDetail.Taxonomy}";
                                    AddQuestionRow(document, tableSections, structureDetail, htmlText, coNo);
                    ...
                    default:
                        {
                            ...
                            var attemptText = $"Attempt ANY {words[structureDetail.AttemptCount ?? 0]} of the following: ({questionType})";
                            var firstOrDefault = paperDetails.FirstOrDefault();
                            AddQuestionRow(document, tableSections, structureDetail, attemptText, string.Empty, paperDetails.Count);
                            AddAttemptRow(document, tableSections, paperDetails);
                        }
                        break;
                }
    ...
    private void AddQuestionRow(Document document, Table table,
        StructureDetail structureDetail, string htmlText, string coNo, int questionCount = 1)
    {
        table.BeginUpdate();
        ...
        document.InsertText(row.Cells[2].Range.Start,
            questionCount > 1 ? $"{coNo} ({structureDetail.Marks}x{structureDetail.AttemptCount})" : $"{coNo} ({structureDetail.Marks})");
    ...
    private void AddMcqRow(Document document, Table table,
        PaperDetail paperDetail, string htmlText, string coNo, int questionCount = 1)
    {
        table.BeginUpdate();
        ...
        // Insert third cell
        document.InsertText(row.Cells[2].Range.Start, $"{coNo}  ({paperDetail.Marks})");
    ...
    {
        var mainRow = AddBlankRow(table);
        SetQuestionRowCellWidths(mainRow);

        // Calculate combined width of 2nd and 3rd cells
        var widthSecond = mainRow.Cells[1].PreferredWidth;
        var widthThird = mainRow.Cells[2].PreferredWidth;
        var totalWidth = widthSecond + widthThird;

        // Create nested table inside 2nd cell
        var tableAttempt = document.Tables.Create(mainRow.Cells[1].Range.Start, 1, 3, AutoFitBehaviorType.FixedColumnWidth);

        tableAttempt.BeginUpdate();

        // Set nested table width equal to combined width of 2nd and 3rd cells
        tableAttempt.PreferredWidthType = WidthType.Fixed;
        ...
        var col2Width = widthSecond - col1Width; // remaining space of second column
        var col3Width = widthThird; // same as main row's third column


        // Adjust column widths inside nested table
        tableAttempt.Rows[0].Cells[0].PreferredWidth = col1Width; // letter column
        tableAttempt.Rows[0].Cells[1].PreferredWidth = col2Width; // question column
        tableAttempt.Rows[0].Cells[2].PreferredWidth = col3Width; // CoNo column

        var index = 0;
        foreach (var detail in paperDetails)
        {
            var attemptRow = AddBlankRow(tableAttempt);
            //SetAttemptRowCellWidths(attemptRow);

            attemptRow.Cells[0].PreferredWidth = col1Width;
            attemptRow.Cells[1].PreferredWidth = col2Width;
            attemptRow.Cells[2].PreferredWidth = col3Width;
            ...
            var coNo = GetCoNoString(detail.CoNo ?? 0, detail.Taxonomy ?? 0);// $"CO{detail?.CoNo} BT{detail.Taxonomy}";
            document.InsertHtmlText(attemptRow.Cells[2].Range.Start, coNo);

            index++;
        }

        // Remove first blank row if added
        if (tableAttempt.Rows.Count > 1)
            tableAttempt.Rows.RemoveAt(0);

        // Make nested table borderless
    ...
    //    var mainRow = AddBlankRow(table);
    //    SetQuestionRowCellWidths(mainRow);
    //    //table.MergeCells(mainRow.Cells[1], mainRow.Cells[2]);

    //    var tableAttempt = document.Tables.Create(mainRow.Cells[1].Range.Start, 1, 3,
    //        AutoFitBehaviorType.AutoFitToContents);
    ...
    //        var coNo = $"CO{detail?.CoNo} BT{detail.Taxonomy}";
    //        document.InsertHtmlText(attemptRow.Cells[2].Range.Start, coNo);
    ...
    private static void SetQuestionRowCellWidths(TableRow row)
    {
        ...
        SetTableCellWidth(row.Cells[1], 6.0F);
        SetTableCellWidth(row.Cells[2], 2.0F);
        /*SetTableCellWidth(row.Cells[0], 0.69F);
    ...
    private string GetCoNoString(int coNo, int taxonomy)
    {
        var coNoString = coNo > 0 ? $"CO{coNo}" : string.Empty;
        var taxonomyString = taxonomy > 0 ? $"BT{taxonomy}" : string.Empty;
        return $"{coNoString} {taxonomyString}";
    }
